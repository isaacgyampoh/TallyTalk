...
export function GroupDetailScreen() {
  ...
  function add() {
    const t = draft.trim()
    if (!t) return
    setTasks((prev) => [...prev, { id: `n-${Date.now()}`, title: t, assignee: 'You', done: false, priority: 'normal' }])
    setDraft('')
  }
        ...
        {tab === 'tasks' ? (
          sorted.map((t) => (
            <div key={t.id} className="flex items-start gap-3 rounded-2xl px-3 py-2.5">
              ...
              <div className="min-w-0 flex-1">
                <p className={`text-[15px] leading-snug ${t.done ? 'text-ink-faint line-through' : 'text-ink'}`}>{t.title}</p>
                <div className="mt-1 flex items-center gap-1.5">
                  <span className="rounded-full bg-wash px-2 py-0.5 text-[11px] font-semibold text-ink-soft">{t.assignee}</span>
                  {t.priority === 'urgent' && <span className="rounded-full bg-urgent/12 px-2 py-0.5 text-[11px] font-semibold text-urgent">Urgent</span>}
                  {t.priority === 'high' && <span className="rounded-full bg-violet/10 px-2 py-0.5 text-[11px] font-semibold text-violet-ink">High</span>}
                </div>
              </div>
            </div>
          ))
          ...
          group.memberList.map((m) => (
            <div key={m.id} className="flex items-center gap-3 rounded-2xl px-3 py-2.5">
              ...
              {m.role === 'administrator' && (
                <span className="rounded-full bg-violet-tint px-2.5 py-0.5 text-[11px] font-semibold text-violet-ink">Admin</span>
              )}
            </div>
          ))
        )}
